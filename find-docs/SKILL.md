...
## Finding GitHub Code Examples

Use `mcp__grep__searchGitHub` to find real code examples from open-source projects:

```bash
mcp__grep__searchGitHub({
  query: "<technology> <specific pattern>",
  language: "<lang>",  // e.g., "typescript", "python"
  max_results: 5
})
```

Combine with documentation research — use docs for API signatures and GitHub search for complete usage patterns.
